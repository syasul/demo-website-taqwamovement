                ...
                <table class="w-full text-left border-collapse" aria-label="Events list">
                    <thead>
                        <tr class="bg-slate-50 border-b border-slate-200 text-xs font-bold text-brand-ink/50 uppercase tracking-wider">
                            <th class="px-6 py-4 w-12 text-center">
                                <input 
                                    type="checkbox" 
                                    @change="let check = $el.checked; selected = check ? [@foreach($events as $event)'{{ $event->id }}',@endforeach] : []"
                                    :checked="selected.length === {{ count($events) }} && {{ count($events) }} > 0"
                    ...
                    <tbody class="divide-y divide-slate-100 text-body text-brand-ink/80">
                        @forelse($events as $event)
                            <tr class="hover:bg-slate-50/50 transition-colors" :class="selected.includes('{{ $event->id }}') ? 'bg-brand-primary/[0.01]' : ''">
